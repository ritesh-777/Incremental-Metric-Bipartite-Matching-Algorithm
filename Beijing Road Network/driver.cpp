...
Result* runInstance(int n, int num_requests, int inst, Scalar min_bound, Scalar max_bound, 
                                                        int dimensions, Scalar delta, std::mt19937& gen, DistCache& cache) {

    // Generate server points and request points for Taxi dataset
    DataSet d("PlotData/"+std::to_string(n)+"/"+std::to_string(inst+1)+"_Point_0.001000_CityRoad_Graph_"+std::to_string(dimensions)+"dim"+".csv", num_requests, dimensions);
    ...
    OnlineDynamicMatching pushRelabel(dimensions, delta, min_bound, max_bound, &cache);
    for (const auto& point : server_points) {
...
std::pair<Scalar, std::pair<double, double>> runInstance_no_opt(int n, int num_requests, Scalar min_bound, Scalar max_bound, 
    ...
    // Generate server points and request points for Taxi dataset
    DataSet d("locations.csv", num_requests, dimensions);
...
int main() {
    ...
    DistCache cache("dists.bin", "nodes.txt", "meta.json");
    ...
    for (int idx = 0; idx < (int)n_values.size(); ++idx) {
        ...
        for (int inst = 0; inst < num_instances; ++inst) {
            ...
            auto result = runInstance(n, n_values[idx], inst, min_bound, max_bound, dimensions, delta, local_gen, cache);
            ...
            std::ofstream pr_file("PlotData/"+std::to_string(n)+"/"+std::to_string(inst+1)+"_PR_"+std::to_string(delta)+"_CityRoad_Graph_"+std::to_string(dimensions)+"dim"+".csv");
            ...
            std::cout << "Instance " << inst+1 << " of n = " << n_values[idx] << " in CityRoad with delta = " << delta << " is done." << std::endl;
        }
