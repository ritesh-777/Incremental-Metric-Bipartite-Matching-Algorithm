...
def generate_data_avg(start_n, end_n, gap, instances, delta, batchSize):
    ...
    for i in range (start_n, end_n+1, gap):
        ...
        for j in range (instances):
            ...
            prpr_csv = f"PlotData/10000/{j+1}_PRPR_{delta}000_CityRoad_Graph_1dim.csv"
            ...
            greedy_csv = f"PlotData/10000/{j+1}_Greedy_{delta}000_CityRoad_Graph_1dim.csv"
            ...
            '''
            qt_csv = f"PlotData/10000/{j+1}_QT_{delta}000_Adult_Eu_6dim.csv"
            ...
            '''
            if i <= 1000 :
                opt_csv = f"PlotData/10000/{j+1}_Opt_{i}_{delta}000_Adult_Eu_6dim.csv"
    ...
    plt.title(f"Average Cost vs. Number of Requests (n) [{delta}]")
    plt.xlabel("n (number of requests)")
    plt.ylabel("Average Cost (PR vs Greedy)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    out1 = f"cost_vs_n_{delta}_adult.png"
    plt.savefig(out1)
    plt.show()
    ...
    #plt.plot(cr_df["n"], cr_df["max_cr_qt"], marker='^', label="Quadtree")
    plt.title(f"Average Match Time per Request vs. Number of Requests (n) [{delta}]")
    plt.xlabel("n (number of requests)")
    plt.ylabel("Average Time per Request (seconds) (PR vs Greedy)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    out2 = f"match_time_vs_n_{delta}_adult.png"
    plt.savefig(out2)
    plt.show()
