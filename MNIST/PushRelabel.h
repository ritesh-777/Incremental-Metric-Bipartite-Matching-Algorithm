    ...
    Scalar getTotalcost() const {
        ...
        for(const Server_np& server : servers) {
            if (server.matched_request_idx != -1) {
                cost += server.location.distance_L1(requests[server.matched_request_idx].location);
            }
        }
        return cost;
    }
...
private:
    ...
    // Helper functions
    int scaled_distance(int level, const Point_np& s, const Point_np& r, Scalar omega) {
        if (level == 0) {
            return std::ceil(2 * s.distance_L1(r) * initial_servers.size() / (epsilon * omega));
        } else {
...
public:
    ...
    void print_matching() const {
        ...
        for (const Edge_np& e : get_matching()) {
            std::cout << "Server " << e.server_idx << " -> Request " << e.request_idx;
            std::cout << " (Distance: " << 
                solutions[0].servers[e.server_idx].location.distance_L1(
