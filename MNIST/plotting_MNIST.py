...
def generate_data_avg(start_n, end_n, gap, instances, delta, batchSize):
    ...
    for i in range (start_n, end_n+1, gap):
        ...
        for j in range (instances):
            ...
            prpr_csv = f"PlotData/10000/{j+1}_PRPR_{delta}000_MNIST_L1_784dim.csv"
            ...
            greedy_csv = f"PlotData/10000/{j+1}_Greedy_{delta}000_MNIST_L1_784dim.csv"
    ...
    plt.grid(True)
    plt.tight_layout()
    out1 = f"cost_vs_n_{delta}_MNIST.png"
    plt.savefig(out1)
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    out2 = f"match_time_vs_n_{delta}_MNIST.png"
    plt.savefig(out2)
    plt.show()
