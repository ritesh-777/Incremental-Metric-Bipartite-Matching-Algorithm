...
public:
    ...
    // Distance function - L1 (Manhattan) distance
    Scalar distance_L1(const Point_np& other) const {
        if (coords.size() != other.coords.size()) {
            std::cerr
            << "ERROR: Point::distance_L1 dimension mismatch: "
            << "this has dim=" << coords.size()
            << ", other has dim=" << other.coords.size()
            << "\n";
            std::abort();   // stops with that message
        ...
        Scalar sum = 0;
        for (size_t i = 0; i < coords.size(); ++i) {
            sum += std::abs(coords[i] - other.coords[i]);
        }
        return sum;
}
