...
def generate_data_avg_with_std(start_n, end_n, gap, instances, delta, batchSize):
    ...
    # Plot mean line
    plt.plot(list_of_n, prpr_cost_avg, color='#1f77b4', linewidth=2.5, label='PRPR (mean)')
    ...
    plt.title(f"Mean ± 1 Std across instances\nParallel-Push-Relabel: Average Cost vs. Number of Requests [{delta}]")
    ...
    out1 = f"MNIST_cost_vs_n_{delta}_with_std.png"
    ...
    # Plot mean line
    plt.plot(list_of_n, prpr_time_avg, color='#ff7f0e', linewidth=2.5, label='PRPR (mean)')
    ...
    plt.title(f"Mean ± 1 Std across instances\nParallel-Push-Relabel: Average Runtime vs. Number of Requests [{delta}]")
    plt.xlabel("Number of requests")
    ...
    out2 = f"MNIST_runtime_vs_n_{delta}_with_std.png"
