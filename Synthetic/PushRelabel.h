...
public:
    OnlineDynamicMatching(int dims, Scalar d = 0.25, Scalar min_val = 0, Scalar max_val = 1) 
        ...
        // halve the number of levels by using the doubled phi-gap
        //mu = std::max(1, mu_orig / 2);   // floor division; ensures at least 1 level
        mu = std::ceil(std::log((2.0/(9*delta))-1) / std::log(3.0));
        epsilon = 1.0 / (2*(std::log(1.0/delta) / std::log(3.0)));
        ...
        //std::cout << "mu: " << mu << ", epsilon: " << epsilon << std::endl;
        //std::cin.get();
        min_bound = min_val;
        max_bound = max_val;
