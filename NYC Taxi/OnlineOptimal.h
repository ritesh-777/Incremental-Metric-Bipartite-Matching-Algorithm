...
public:
    ...
    void processRequest(const Point_np& r) {
        ...
        for (int r_j = 0; r_j < k; ++r_j) {
            for (int s = 0; s < n; ++s) {
                if (match_M_star[r_j] != s) {
                    Scalar cost = t * servers[s].distance(requests[r_j]) - y_S[s] - y_R[r_j];
        ...
        for (int s = 0; s < n; ++s) {
            Scalar cost = t * servers[s].distance(requests[k]) - y_S[s];
        ...
        for (size_t i = 0; i < path.size(); i += 2) {
            ...
            y_R[r] -= (t - 1) * servers[s].distance(requests[r]);
        }
    ...
    Scalar getTotalCost() const {
        ...
        for (size_t k = 0; k < requests.size(); ++k) {
            ...
            total += servers[s].distance(requests[k]);
        }
        return total;
    }
};
