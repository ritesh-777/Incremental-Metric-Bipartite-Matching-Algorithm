...
def run_experiments_greedy(
    ...
    for sub in subfolders:
        ...
        for ds_id in range(1, num_datasets + 1):
            # -------------------------------
            # build file names
            input_file = f"{ds_id}_Point_{delta:.6f}_Real_Eu_{dim}dim.csv"
            output_file = f"{ds_id}_Greedy_{delta:.6f}_Real_Eu_{dim}dim.csv"
...
def run_experiments_pr(
    ...
    for sub in subfolders:
        ...
        for ds_id in range(1, num_datasets + 1):
            ...
            # build file names
            input_file = f"{ds_id}_Point_{delta:.6f}_Real_Eu_{dim}dim.csv"
            output_file = f"{ds_id}_PRPR_{delta:.6f}_Real_EU_{dim}dim.csv"
