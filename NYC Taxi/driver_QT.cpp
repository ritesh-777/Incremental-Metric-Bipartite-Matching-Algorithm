...
Result* runInstance(int n, int num_requests, int inst, Scalar min_bound, Scalar max_bound, 
    ...
    DataSet d("PlotData/"+std::to_string(n)+"/"+std::to_string(inst+1)+"_Point_"+std::to_string(delta)+"_Real_Eu_"+std::to_string(dimensions)+"dim"+".csv", num_requests);
...
int main() {
    ...
    const Scalar min_bound = -80;//0.0;
    const Scalar max_bound = 50;//100.0;
    ...
    for (int idx = 0; idx < (int)n_values.size(); ++idx) {
        ...
        for (int inst = 0; inst < num_instances; ++inst) {
            ...
            std::ofstream qt_file("PlotData/"+std::to_string(n_values[idx])+"/"+std::to_string(inst+1)+"_QT_"+std::to_string(delta)+"_Real_Eu_"+std::to_string(dimensions)+"dim"+".csv");
