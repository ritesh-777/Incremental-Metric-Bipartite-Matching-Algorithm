...
def generate_data_avg(start_n, end_n, gap, instances, delta, batchSize):
    ...
    for i in range (start_n, end_n+1, gap):
        ...
        for j in range (instances):
            ...
            prpr_csv = f"PlotData/10000/{j+1}_PRPR_{delta}000_Real_Eu_2dim.csv"
            ...
            prpr_cost_iavg.append(prpr_df.loc[:, 'cost'].iloc[-1]/i)
            #prpr_time_iavg.append(prpr_df.loc[:, 'run_time'].mean())
            print(prpr_cost_iavg)
            
            
            greedy_csv = f"PlotData/10000/{j+1}_Greedy_{delta}000_Real_Eu_2dim.csv"
            ...
            qt_csv = f"PlotData/10000/{j+1}_QT_{delta}000_Real_Eu_2dim.csv"
            ...
            dyneuc_csv = f"PlotData/10000/{j+1}_DynEuc_B16_{delta}000_Real_Eu_2dim.csv"
            ...
            dyneuc_df['run_time'] = dyneuc_df['execution_time'].astype(float)
            # Partition into k groups using integer division of index
            ...
            #selected_dyneuc_time_df = dyneuc_df['run_time'].iloc[take_every-1::take_every]
            dyneu_cost_iavg.append(selected_dyneuc_cost_df.iloc[-1]/i)
            #dyneu_time_iavg.append(selected_dyneuc_time_df.mean())
            #dyneu_cost_iavg.append(dyneuc_df.loc[:, 'cost'].mean())
            dyneu_time_iavg.append(dyneuc_df.loc[:, 'execution_time'].mean())
            
            '''
            if i <= 1000 :
                opt_csv = f"PlotData/10000/{j+1}_Opt_{i}_{delta}000_Real_Eu_2dim.csv"
        ...
        dyneu_cost_avg.append(sum(dyneu_cost_iavg)/float(len(dyneu_cost_iavg)))
        dyneu_time_avg.append(sum(dyneu_time_iavg)/float(len(dyneu_time_iavg)))
    ...
    plt.plot(list_of_n, dyneu_cost_avg, marker='d', label="Dynamic Euclidean")
    #plt.plot([1000], opt_cost_avg, marker='*', label="Optimal")
    ...
    plt.grid(True)
    plt.tight_layout()
    out1 = f"cost_vs_n_{delta}_real.png"
    plt.savefig(out1)
    plt.show()
    ...
    plt.plot(list_of_n, dyneu_time_avg, marker='d', label="Dynamic Euclidean")
    ...
    plt.grid(True)
    plt.tight_layout()
    out2 = f"match_time_vs_n_{delta}_real.png"
    plt.savefig(out2)
    plt.show()
